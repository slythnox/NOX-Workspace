...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float dn = d / r;
    float spot = clamp(1.0 - pow(dn, uSpotlightSoftness), 0.0, 1.0) * uSpotlightOpacity;
    float stripe = fract(uvMod.x * max(uBlindCount, 1.0));
    if (uShineFlip > 0.5) stripe = 1.0 - stripe;

    vec3 col = base * spot * (1.0 - stripe * 0.75);
    col += (rand(gl_FragCoord.xy + iTime) - 0.5) * uNoise * spot;
...
`;
    ...
    const resize = () => {
      ...
      if (firstResizeRef.current) {
        ...
        uniforms.iMouse.value = [-1000, -1000];
        mouseTargetRef.current = [-1000, -1000];
      }
    };
    ...
    const loop = (t: number) => {
      ...
      // Auto-reveal spotlight animation 2 seconds after mounting if the user hasn't hovered yet
      if (!hasInteractedRef.current) {
        const totalElapsed = (performance.now() - startTime) / 1000;
        if (totalElapsed > 2.0 && totalElapsed <= 4.0) {
          const progress = (totalElapsed - 2.0) / 2.0; // Interpolate over 2.0s
          const tSmooth = progress * progress * (3.0 - 2.0 * progress); // smoothstep
          
          const cx = gl.drawingBufferWidth / 2;
          const cy = gl.drawingBufferHeight / 2;
          
          mouseTargetRef.current[0] = -1000.0 + (cx + 1000.0) * tSmooth;
          mouseTargetRef.current[1] = -1000.0 + (cy + 1000.0) * tSmooth;
        }
      }
