...
export const DotField: React.FC<DotFieldProps> = ({
  ...
  // Cache props in a ref to prevent recreation of requestAnimationFrame loop and resize/mouse listeners
    ...
    const render = () => {
      ...
      for (let c = 0; c < cols; c++) { 
        for (let r = 0; r < rows; r++) {
          const origX = c * dotSpacing; 
          let origY = r * dotSpacing;
          ...
          if (dist < cursorRadius && dist > 0) { 
            const force = Math.pow(1 - dist/cursorRadius, 2) * cursorForce * bulgeStrength; 
            if (bulgeOnly) { 
              drawX -= (dx/dist)*force*1.5; 
              drawY -= (dy/dist)*force*1.5; 
            } else { 
              drawX += (dx/dist)*force; 
              drawY += (dy/dist)*force; 
            } 
          ...
          let opacity = 0.18; 
          if (dist < cursorRadius) opacity = 0.18 + (1 - dist/cursorRadius)*0.65;
          let radius = dotRadius; 
          if (sparkle && Math.random() > 0.996) { 
            radius *= 2.5; 
            opacity = 1; 
          }
